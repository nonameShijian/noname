/// <reference path="./status.d.ts" />

/// <reference path="./ai.d.ts" />

/// <reference path="./ArrayEx.d.ts" />
/// <reference path="./DateEx.d.ts" />
/// <reference path="./HTMLDivElementEx.d.ts" />
/// <reference path="./windowEx.d.ts" />

/// <reference path="./game.d.ts" />

/// <reference path="./get.d.ts" />

/// <reference path="./brawl.d.ts" />

/// <reference path="./AIConfig.d.ts" />
/// <reference path="./CardConfig.d.ts" />
/// <reference path="./CharacterConfig.d.ts" />
/// <reference path="./develop.d.ts" />
/// <reference path="./ExInterface.d.ts" />
/// <reference path="./ExtensionConfig.d.ts" />
/// <reference path="./import-card.d.ts" />
/// <reference path="./import-character.d.ts" />
/// <reference path="./SkillConfig.d.ts" />
/// <reference path="./type.d.ts" />

/// <reference path="./click.d.ts" />
/// <reference path="./create.d.ts" />
/// <reference path="./ui.d.ts" />

/// <reference path="./button.d.ts" />
/// <reference path="./card.d.ts" />
/// <reference path="./cheat.d.ts" />
/// <reference path="./client.d.ts" />
/// <reference path="./content.d.ts" />
/// <reference path="./control.d.ts" />
/// <reference path="./dialog.d.ts" />
/// <reference path="./event.d.ts" />
/// <reference path="./filter.d.ts" />
/// <reference path="./init.d.ts" />
/// <reference path="./lib.d.ts" />
/// <reference path="./LocalGameConfigMenu.d.ts" />
/// <reference path="./message.d.ts" />
/// <reference path="./nodews.d.ts" />
/// <reference path="./player.d.ts" />
/// <reference path="./sort.d.ts" />
/// <reference path="./ws.d.ts" />

/// <reference path="./cordova.d.ts" />
/// <reference path="./cordova-plugin-dialogs.d.ts" />
/// <reference path="./cordova-plugin-file.d.ts" />
/// <reference path="./cordova-plugin-file-transfer.d.ts" />
/// <reference path="./cordova-plugin-local-notifications.d.ts" />
/// <reference path="./electron.d.ts" />
/// <reference path="./ws.d.ts" />

/** 先行测试，测试用 */
/*declare module NonameGame {
    export var lib: Lib;
    export var status: Status;
    export var game: Game;
    export var ui: UI;
    export var get: Get;
    export var ai: AI;
}*/