...
※打包前请先将无名杀本体的完整包复制到根目录的`noname/`文件夹下
...
3. 将无名杀本体的完整包复制到根目录的`noname/`文件夹下；

4. 在文件夹中运行`npm run packageDarwin`（Intel芯片使用`npm run packageDarwinIntel`）打包应用；

5. 打包完成后，在新出现的文件夹中找到“无名杀.app”，移动到“应用程序”即可。
